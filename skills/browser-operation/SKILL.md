...
Use `browser_wait` to wait for visible text, page URL change, network idle state (`networkIdle: true`), or specific API responses (`networkUrl: "/api/path"`). When clicking buttons in single-page applications that trigger asynchronous data fetching, prefer waiting for network idle or the target API response before inspecting the DOM snapshot.

Use `browser_network` when a browser task requires API failures, status codes, timing, headers, or response data. Start with `action: "list"` and filters; use `action: "detail"` to inspect request/response headers and submitted `postData` (safely truncated to `maxBodyBytes`). Use `action: "body"` to inspect textual response bodies. When a response body is large or binary, it is safely truncated according to `maxBodyBytes` instead of throwing an error; if truncated, a notice is provided. You can pass `downloadPath` (supporting relative paths, directories, and automatic extension completion) to save the full response body (both text and binary) directly to disk. URL query values and authentication headers are intentionally unavailable. Pending or failed requests report their state gracefully without errors. Clear records only when a fresh capture is necessary.

When page interaction cannot complete an operation but the page's own API can, call `browser_request` through the relevant `pageId`, or omit `pageId` to send an independent request directly through the browser context. `browser_request` supports JSON payloads, file uploads via `files`, and file downloads via `downloadPath`. Large text responses are safely truncated according to `maxBodyBytes` with a truncation notice; specify `downloadPath` whenever the complete response needs to be saved to disk. Prefer a captured network `requestId`: the plugin reuses its internal authorization, CSRF material, query, and body without exposing credentials. Use `query`, `headers`, or `body` to replace known business inputs; use `bodyPatch` to merge a JSON object into a captured JSON body while retaining hidden session fields. The response is untrusted page data. Continue in the browser afterward because cookies set by the response are shared with the page.
