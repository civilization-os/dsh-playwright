...
插件在用户明确提出保存或更新操作手册时，才根据本次经过验证的浏览器轨迹生成草稿。任务成功本身不会创建、修改或启用操作手册。操作手册与 browser-operation 核心 Skill 分开保存，避免一次站点探索改变所有浏览任务的规则。

操作手册只记录可复用信息：

- 适用的站点 origin、路径模式和任务目标
- 每一步所在的页面区域、元素角色和可访问名称
- 需要由用户任务提供的参数
- 操作前置条件和操作后的成功断言
...
1. 模型正常探索页面并执行操作。
2. 至少一个关键动作携带预期结果，并由 Host 验证成功。
3. Host 只在当前会话中保留可用于总结的轨迹，不自动持久化操作手册。
4. 用户明确提出“保存这次流程”“生成操作手册”或同等要求后，模型调用工具生成草稿。
5. 用户在设置页审阅并启用草稿。
...
| browser_runbook_save | 响应用户的明确要求，根据 Host 已验证的当前会话轨迹生成或修订草稿 |
