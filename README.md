...
第一次完成一个经过验证的页面任务后，插件可以根据本次浏览器轨迹生成一份操作手册草稿。操作手册与 browser-operation 核心 Skill 分开保存，避免一次站点探索改变所有浏览任务的规则。
...
4. 手册步骤失效时回到局部 snapshot 探索，并在成功后生成修订草稿。

设置页提供“操作手册”区域，显示名称、站点、最近成功时间、成功次数、版本、启用状态和失效次数，并支持查看、停用、删除和重新学习。默认策略为“成功后生成草稿”，不自动覆盖已启用版本。
...
| browser_runbook_finish | 声明任务结果，并根据 Host 已验证的轨迹生成或修订草稿 |
